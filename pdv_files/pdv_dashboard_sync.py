...
requests.post = lambda *a, **kw: _orig_post(*a, **{**kw, "verify": kw.get("verify", False)})
...
VIDEO_RETRY_MAX_POR_EXECUCAO = 10

# Spy file usa SBT (SubTotal) para marcar fechamento de cupom
# Formato: 15:30:56:SBT | Cod: 227945 | Descricao: Operador | VlTotal: 278.81|
FECHACUPOM_RE = re.compile(r":SBT \| Cod: (\S+) \| Descricao:.*?\| VlTotal: ([\d.]+)\|")
...
def coletar_vendas(args, dt=None):
    ...
    # Agrupa por número de cupom e usa o ÚLTIMO VlTotal de cada um
    # (um mesmo cupom pode ter múltiplos SBT por divisão de pagamento)
    cupom_totais = {}
    with caminho.open("r", encoding="latin-1", errors="replace") as handle:
        for linha in handle:
            ...
            if not match:
                continue
            num_cupom = match.group(1)
            vlTotal = float(match.group(2))
            cupom_totais[num_cupom] = vlTotal

    total = sum(cupom_totais.values())
    cupons = len(cupom_totais)
    return total, cupons
...
def enviar_vendas(args):
    ...
    hoje = datetime.now()  # sempre data real do sistema, não a data do bot Telegram
...
_STREAMER_ENV = "/etc/pdv-video-streamer.env"
_STREAMER_SERVICE = "pdv-video-streamer"


def _ler_streamer_env():
    vals = {}
    try:
        for linha in Path(_STREAMER_ENV).read_text().splitlines():
            linha = linha.strip()
...
def _sincronizar_camera_streamer(rcfg):
    novo = {}
    if rcfg.get("camera_host"):
        novo["IMHDX_HOST"] = rcfg["camera_host"]
    if rcfg.get("camera_user"):
        novo["IMHDX_USER"] = rcfg["camera_user"]
    if rcfg.get("camera_pass"):
        novo["IMHDX_PASS"] = rcfg["camera_pass"]
    if rcfg.get("camera_channel") is not None:
        novo["IMHDX_CHANNEL"] = str(int(rcfg["camera_channel"]))
    if rcfg.get("auditoria_api_key") and rcfg.get("auditoria_provedor", "gemini") == "gemini":
        novo["GEMINI_API_KEY"] = rcfg["auditoria_api_key"]
    if not novo:
        return

    atual = _ler_streamer_env()
    if all(atual.get(k) == v for k, v in novo.items()):
        return  # sem mudança, não reinicia

    merged = dict(atual)
    merged.update(novo)
    conteudo = "\n".join(f"{k}={v}" for k, v in merged.items()) + "\n"
    Path(_STREAMER_ENV).write_text(conteudo)
    subprocess.run(["systemctl", "restart", _STREAMER_SERVICE], timeout=10, check=False)
...
def main():
    ...
    try:
        ...
        _rcfg = _cfg_loader.fetch()
        if _rcfg.get("camera_host"):
            args.imhdx_host = _rcfg["camera_host"]
        if _rcfg.get("camera_user"):
            args.imhdx_user = _rcfg["camera_user"]
        if _rcfg.get("camera_pass"):
            args.imhdx_pass = _rcfg["camera_pass"]
        if _rcfg.get("camera_channel") is not None:
            args.imhdx_channel = int(_rcfg["camera_channel"])
        _sincronizar_camera_streamer(_rcfg)
    except Exception:
        pass
